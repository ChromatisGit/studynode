...
export function Breadcrumbs() {
  const { hasTopicContext, isLibraryRoute, subject, courseId, topic, chapter } =
    useRouteContext();
  ...
  if (!isLibraryRoute && courseId) {
    ...
    const topicData = overview?.topics.find((entry) => entry.slug === topic);
    topicLabel = topicData?.title ?? topicLabel;
    const chapterData = topicData?.chapters.find((entry) => entry.slug === chapter);
